...
def get_btc_price():
    ...
    try:
        ...
        resp = requests.get('https://api.gateio.ws/api/v4/futures/usdt/tickers?contract=BTC_USDT',
                        headers=headers, timeout=10)
        data = resp.json()
        if isinstance(data, list) and data:
            return float(data[0]['last'])
    except Exception as e:
        print(f"❌ Gate.io BTC 失败: {e}")
    return None


def get_eth_price():
    """获取ETHUSDT价格（Gate.io API，无缓存）"""
    try:
        headers = {
            'Cache-Control': 'no-store, no-cache, must-revalidate, max-age=0',
            'Pragma': 'no-cache',
        ...
        resp = requests.get('https://api.gateio.ws/api/v4/futures/usdt/tickers?contract=ETH_USDT',
                        headers=headers, timeout=10)
        data = resp.json()
        if isinstance(data, list) and data:
            return float(data[0]['last'])
    except Exception as e:
        print(f"❌ Gate.io ETH 失败: {e}")
    return None
...
def main():
    """主监控循环"""
    print("=" * 60)
    print("BTC + ETH 多时间框架监控系统启动（Gate.io API 无缓存）")
    ...
    last_btc = None
    last_eth = None
    last_status_push = 0   # 上次状态推送时间（unix）
    last_btc_push_price = None  # 上次推送的BTC价格（用于0.3%变动推送）
    last_eth_push_price = None  # 上次推送的ETH价格
    ...
    while True:
        try:
            ...
            print(f"\n[{now_str}] BTC: {btc_price:.1f} {btc_change_str} | ETH: {eth_price:.2f} {eth_change_str}")

            # 价格变动>0.3%推送企微（不只等关键位）
            if last_btc_push_price and abs(btc_price - last_btc_push_price) / last_btc_push_price >= 0.003:
                msg = f"""📈 BTC 价格变动提醒
当前价：${btc_price:.1f}
涨跌幅：{(btc_price - last_btc_push_price) / last_btc_push_price * 100:+.2f}%
时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
                send_wechat_alert(msg)
                last_btc_push_price = btc_price

            if last_eth_push_price and abs(eth_price - last_eth_push_price) / last_eth_push_price >= 0.003:
                msg = f"""📈 ETH 价格变动提醒
当前价：${eth_price:.2f}
涨跌幅：{(eth_price - last_eth_push_price) / last_eth_push_price * 100:+.2f}%
时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
                send_wechat_alert(msg)
                last_eth_push_price = eth_price

            # 首次运行初始化推送价格
            if last_btc_push_price is None:
                last_btc_push_price = btc_price
            if last_eth_push_price is None:
                last_eth_push_price = eth_price

            # 每30分钟推送一次状态摘要
            if now_ts - last_status_push >= 1800:
                msg = f"""📊 BTC+ETH 监控状态摘要
